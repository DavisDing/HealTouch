...
public class UserPage {
  ...
  private final TableView<UserService.Staff> table = new TableView<UserService.Staff>();

  public UserPage(AppServices services, UserSession session) {
    this.services = services;
    this.session = session;
  }

  public Parent node() {
    Button add = new Button("新增用户");
    add.setOnAction(e -> add());
    Button toggle = new Button("在职/停用");
    toggle.setOnAction(
        e -> {
          try {
            UserService.Staff s = table.getSelectionModel().getSelectedItem();
            if (s == null) throw new IllegalArgumentException("请选择用户");
            services.users.setActive(session, s.id, !s.active);
            load();
          } catch (Exception ex) {
            Ui.error(ex);
          }
    ...
    VBox page = Ui.page("用户管理", new HBox(10, add, toggle), table);
    load();
    return page;
  }

  private TableColumn<UserService.Staff, String> col(
      String label, java.util.function.Function<UserService.Staff, String> getter) {
    TableColumn<UserService.Staff, String> c = new TableColumn<UserService.Staff, String>(label);
  ...
  private void load() {
    try {
      table.setItems(FXCollections.observableArrayList(services.users.listAll()));
    } catch (Exception e) {
      Ui.error(e);
    }
  }
