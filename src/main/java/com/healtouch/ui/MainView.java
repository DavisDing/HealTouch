...
public class MainView {
  ...
  public MainView(AppServices services, UserSession session, Runnable logout) {
    ...
    Label brand = new Label("HealTouch");
    ...
    HBox header = new HBox(14, brand, spacer, who, exit);
    header.setPadding(new Insets(12, 18, 12, 18));
    header.setStyle("-fx-background-color: #f3f6f8;");
    root.setTop(header);
    VBox nav = new VBox(2);
    nav.setPadding(new Insets(12));
    nav.setPrefWidth(180);
    nav.setStyle("-fx-background-color: #fbfbfb;");
    add(nav, "首页", () -> show(new DashboardPage(services, session, this).node()));
    if (can(Permission.PATIENT_VIEW))
      add(nav, "患者管理", () -> show(new PatientPage(services, session).node()));
    ...
    if (can(Permission.SYSTEM_MANAGE)) {
      ...
      label.setStyle("-fx-font-weight: bold; -fx-padding: 10 8 4 8;");
      ...
      add(nav, "治疗项目管理", () -> show(new CatalogPage(services, session).node()));
    }
    root.setLeft(nav);
  ...
  public void openPatients() {
    show(new PatientPage(services, session).node());
  }

  public void openTreatment() {
    show(new TreatmentPage(services, session).node());
  }

  public void openBills() {
    show(new BillPage(services, session).node());
  }

  public void openDeposits() {
    show(new DepositPage(services, session).node());
  }

  private boolean can(Permission p) {
    try {
      Authorization.require(session, p);
      return true;
